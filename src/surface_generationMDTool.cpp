...
void Surface_GenerationMDTool_Plugin::createCages(PFP2::MAP* object)
{
    ...
    d0 = map->phi1(d0);
    position[d0] = PFP2::VEC3(min[0] - (max[0]-min[0])/20, (max[1]+min[1])/2, min[2]);
    m_cages.back().push_back(d0);   //min

    d0 = map->phi1(d0);
    position[d0] = PFP2::VEC3(min[0] - (max[0]-min[0])/20, max[1] + (max[1]-min[1])/20, min[2]);

    d0 = map->phi1(d0);
    position[d0] = PFP2::VEC3((max[0]+min[0])/2, max[1] + (max[1]-min[1])/20, min[2]);
    ...
    d2 = map->phi1(d2);
    position[d2] = PFP2::VEC3((max[0]+min[0])/2, min[1] - (max[1]-min[1])/20, min[2]);
    ...
    m_cages.push_back(std::deque<Dart>());
    d3 = map->phi1(d3);
    position[d3] = PFP2::VEC3(max[0] + (max[0]-min[0])/20, (max[1]+min[1])/2, min[2]);
    m_cages.back().push_back(d3);   //max

    d3 = map->phi1(d3);
    position[d3] = PFP2::VEC3(max[0] + (max[0]-min[0])/20, min[1] - (max[1]-min[1])/20, min[2]);
    d3 = map->phi1(d3);
